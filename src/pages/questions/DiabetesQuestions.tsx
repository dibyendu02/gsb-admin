...
export default function DiabetesQuestions() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (currentQuestionId) {
        await putData(
          `/api/diabetesQuestion/${currentQuestionId}`,
          newQuestion,
          auth.token,
          null
        );
      } else {
        await postData("/api/diabetesQuestion", newQuestion, auth.token, null);
      }
  ...
  const handleDelete = async (questionId: string) => {
    ...
    try {
      await deleteData(`/api/diabetesQuestion/${questionId}`, auth.token);
      getQuestions(); // Refresh the question list
    } catch (err) {
