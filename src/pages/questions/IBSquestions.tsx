...
export default function IBSquestions() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (currentQuestionId) {
        await putData(
          `/api/IBSquestions/${currentQuestionId}`,
          newQuestion,
          auth.token,
          null
        );
      } else {
        await postData("/api/IBSquestions", newQuestion, auth.token, null);
      }
  ...
  const handleDelete = async (questionId: string) => {
    ...
    try {
      await deleteData(`/api/IBSquestions/${questionId}`, auth.token);
      getQuestions(); // Refresh the question list
    } catch (err) {
