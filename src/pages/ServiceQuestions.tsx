...
export default function ServiceQuestions() {
  ...
  const getQuestions = async () => {
    try {
      const response = await getData("/api/serviceQuestions", auth.token);
  ...
  const handleSave = async () => {
    ...
    try {
      if (currentQuestionId) {
        await putData(
          `/api/serviceQuestions/${currentQuestionId}`,
          newQuestion,
          auth.token,
          null
        );
      } else {
        await postData("/api/serviceQuestions", newQuestion, auth.token, null);
      }
  ...
  const handleDelete = async (questionId: string) => {
    ...
    try {
      await deleteData(`/api/serviceQuestions/${questionId}`, auth.token);
      getQuestions(); // Refresh the question list
    } catch (err) {
